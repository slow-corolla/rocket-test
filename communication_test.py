...
SENDER_PASS   = os.environ.get("GMAIL_APP_PASSWORD", "")       # Gmail App Password (not your login password)
...
OUT_DIR.mkdir(exist_ok=True)    # Unless the folder exists, create it
...
def within_operating_hours():
    # Return True if the current hour is between 1 AM and 12 PM inclusive.
    now = datetime.now().time()
    return 1 <= now.hour <= 12 # Previous script used and instead of or; "or" allows this to wrap around midnight
...
def main(): 
    raise RuntimeError("This is a test error to verify the alert system is working correctly.")
